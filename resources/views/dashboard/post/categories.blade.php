...
@section('content') 
<h1>Categories</h1>
<a href="{{ route('post.createCat') }}" class="btn btn-success">
  Create
</a>
  ...
  <tbody>
    @foreach ($categories as $categories)
      <tr>
        ...
        <td>{{ $categories['url_clean'] }}</td>
       
        <td>
          <a href="{{ route('post.editCat', $categories->id) }}" class="btn btn-secondary">Edit</a>
        </td>
        <td>
            <button data-bs-toggle="modal" data-bs-target="#deleteModal" data-bs-id="{{ $categories->id }}" type="submit" class="btn btn-danger">Delete</button>
        </td>
      </tr>
    @endforeach
